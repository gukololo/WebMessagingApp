...
// SSE event kuyruğu: "event: X\ndata: {...}\n\n" formatında saklanır
static vector<string> g_events;
...
// clients -> JSON (bu fonksiyonu çağırmadan önce kilitle!)
static string users_json_from_clients_locked() {
    string out = "[";
    bool first = true;
    for (auto& c : clients) {
...
// Kullanıcı varsa referansını döndürür; yoksa oluşturup referans döndürür.
// NOT: Çağrıldığı yer kilitliyse (lock_guard ile), burada ayrıca kilide gerek yok.
static Client& ensure_client(const string& name) {
    for (Client& c : clients) {
        if (c.getName() == name) return c;      // vektör içindeki elemana referans
    }
    clients.emplace_back(name);                 // yeni Client
    clients.back().setIsOnline(false);          // başlangıçta offline
    return clients.back();
}

// SSE kuyruğuna event ekle + bekleyenleri uyandır
static void push_sse(const string& event_name, const string& json_payload) {
    lock_guard<mutex> lk(g_m);
    g_events.push_back("event: " + event_name + "\n" "data: " + json_payload + "\n\n");
    g_cv.notify_all();
...
int main() {
    ...
    server.Post("/login", [](const httplib::Request& req, httplib::Response& res) {
        if (!req.has_param("username")) {
            res.status = 400; res.set_content("Username is required", "text/plain"); return;
        }
        username = req.get_param_value("username");
        ...
        if (changed) {
            string json;
            { lock_guard<mutex> lk(g_m); json = users_json_from_clients_locked(); }
            push_sse("users", json); // tüm istemcilere anında
        }
    ...
    server.Get("/events", [](const httplib::Request& req, httplib::Response& res) {
        ...
        // 3 argümanlı overload: (mime, provider, releaser)
        res.set_chunked_content_provider(
            "text/event-stream",
            // provider: çağrıldığında sıradaki eventleri ve heartbeat gönderir
            [last = size_t{ 0 }](size_t /*offset*/, httplib::DataSink& sink) mutable {
                vector<string> batch;
                {
                    unique_lock<mutex> lk(g_m);
                    if (last >= g_events.size()) g_cv.wait_for(lk, seconds(2)); // 2sn heartbeat
                    while (last < g_events.size()) batch.push_back(g_events[last++]);
                }
                for (const auto& ev : batch) {
                    if (!sink.write(ev.data(), ev.size())) return false;
    ...
    server.Post("/send", [](const httplib::Request& req, httplib::Response& res) {
        ...
        if (begins_with("/add ")) {
            ...
            }
            {
                lock_guard<mutex> lk(g_m);
                Client& me = ensure_client(user);
                ensure_client(target);           // hedef kayıt yoksa oluştur (offline olabilir)
                me.addDestinationName(target);
            }
            string j = string("{\"text\":\"") + json_escape("Added " + target + " to your destinations") +
                "\",\"to\":[\"" + json_escape(user) + "\"]}";
        ...
        // Normal mesaj: sadece gönderenin hedef listesine gitsin
        vector<string> to_list;
        {
            lock_guard<mutex> lk(g_m);
            Client& me = ensure_client(user);
            to_list = me.getDestinationNames(); // kopya
        }
        ...
        // {"user":"Alice","text":"...","to":["Bob","Charlie"]}
        string j = string("{\"user\":\"") + json_escape(user) + "\",\"text\":\"" + json_escape(text) +
            "\",\"to\":" + json_array_of_strings(to_list) + "}";
        push_sse("msg", j);
        res.set_content("OK", "text/plain");
    ...
    // POST /leave -> pencere/sekme kapanırken anında offline bildirimi
    server.Post("/leave", [](const httplib::Request& req, httplib::Response& res) {
        if (!req.has_param("user")) { res.status = 400; res.set_content("Missing user", "text/plain"); return; }
        const string who = req.get_param_value("user");
